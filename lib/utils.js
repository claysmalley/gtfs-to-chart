...
const getStationsFromTrip = async (trip, defaultTimezone, agencyKey) => {
...
const getDataforChart = async (config, page, agencyTimezone) => {
    ...
    for (const i in routeIds) {
      ...
      formattedTrips.push(...trips.map(trip => {
        return {
          ...trip,
          route_id: routeIds[i],
          route_long_name: route ? route.route_long_name : trip.route_long_name,
        };
      }));
    }
  ...
  let stations = await getStationsFromTrip(longestTrip, agencyTimezone, agencyKey);
  const tripDistance = max(map(stations, 'distance'));
  ...
  if (size(directionGroups) > 1) {
    ...
    const stationsOppositeDirection = await getStationsFromTrip(longestTripOppositeDirection, agencyTimezone, agencyKey);
