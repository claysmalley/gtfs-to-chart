...
const getStationsFromTrip = async (trip, defaultTimezone, config) => {
 ...
 */
const getDataforChart = async (config, page) => {
  ...
  const agency = await db.all('SELECT DISTINCT agency_timezone FROM agency');
  ...
  let stations = await getStationsFromTrip(longestTrip, agencyTimezone, config);
  const tripDistance = max(map(stations, 'distance'));
  ...
  if (size(directionGroups) > 1) {
    ...
    const stationsOppositeDirection = await getStationsFromTrip(longestTripOppositeDirection, agencyTimezone, config);
...
export async function generateChartHTML(config, urlSlug) {
  ...
  const chartData = await getDataforChart(config, route);
