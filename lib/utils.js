...
const getDataforChart = async (config, page, agencyTimezone) => {
  ...
  for (let day = 0; day < days.length; day++) {
    ...
    if (calendar.length === 0) {
      throw new Error(`No calendars found for route ${page.label} on ${moment(chartDates[day], 'YYYYMMDD').format('MMM D, YYYY')}`);
    }
  }
  ...
  for (let day = 0; day < days.length; day++) {
    ...
    for (const i in routeIds) {
      ...
      const route = find(routeList, ['route_id', routeIds[i]]);
