...
import { routeList, amtrakStations, amtrakRoutes } from './data.js';
...
const getStationsFromTrip = async trip => {
  ...
  return trip.stoptimes.map((stoptime, index) => {
    ...
    const formattedStop = {
      ...stop,
      ...find(amtrakStations, ['stop_id', stop.stop_id])
    };
 ...
 */
const getDataforChart = async (config, route) => {
  const routeIds = route.routes;
  const days = [-2, -1, 0, 1, 2, 3, 4, 5, 6];
  ...
  for (let day = 0; day < days.length; day++) {
    ...
    if (calendar.length === 0) {
      throw new Error(`No calendars found for route ${route.label} on ${moment(chartDates[day], 'YYYYMMDD').format('MMM D, YYYY')}`);
    }
  }
  ...
  for (let day = 0; day < days.length; day++) {
    ...
    for (const i in routeIds) {
      const trips = await db.all(`SELECT service_id, trip_id, trip_headsign, trip_short_name, direction_id, shape_id FROM trips where route_id = ? AND service_id IN (${serviceIds[day].map(() => '?').join(', ')})`, [
        routeIds[i],
        ...serviceIds[day]
      ]);
      const amtrakRoute = find(amtrakRoutes, ['route_id', routeIds[i]]);
      formattedTrips.push(...trips.map(trip => {
        return {
          ...trip,
          route_id: routeIds[i],
          route_long_name: amtrakRoute.route_long_name,
        };
      }));
    }
  ...
  if (tripsByDay.flat().length === 0) {
    throw new Error(`No trips found for route ${route.label} on ${moment(chartDates[0], 'YYYYMMDD').format('MMM D, YYYY')}`);
  }
  ...
  for (let day = 0; day < days.length; day++) {
    await Promise.all(tripsByDay[day].map(async trip => {
      ...
      trip.stoptimes = stoptimes.filter(stoptime => isTimepoint(stoptime)).map(
        (stoptime) => {
          stoptime['arrival_time_utc'] = parseTime(stoptime.arrival_time, chartDates[day], 'America/New_York');
          stoptime['departure_time_utc'] = parseTime(stoptime.departure_time, chartDates[day], 'America/New_York');
          return stoptime;
        }
      );
      trip.start_day = day;
...
export async function generateChartHTML(config, urlSlug) {
  const route = find(routeList, ['url_slug', urlSlug]);
