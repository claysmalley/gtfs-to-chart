 ...
 */
function calculateDistanceMi(lat1, lon1, lat2, lon2, minStopDistance, maxStopDistance) {
  ...
  dist = dist * 60 * 1.1515;
  if (minStopDistance) {
    dist = Math.max(minStopDistance, dist);
  }
  if (maxStopDistance) {
    dist = Math.min(maxStopDistance, dist);
  }
  return dist;
}
...
const getStationsFromTrip = (trip, defaultTimezone, config) => {
  const agencyKey = config.agencies[0].agency_key;
  const noStopCode = config.noStopCode;
  const minStopDistance = config.minStopDistance;
  const maxStopDistance = config.maxStopDistance;
  ...
  return trip.stoptimes.map((stoptime, index) => {
    ...
    if (!hasShapeDistance) {
      if (index === 0) {
        ...
        const distanceFromPreviousStation = calculateDistanceMi(stop.stop_lat, stop.stop_lon, previousStationCoordinates.stop_lat, previousStationCoordinates.stop_lon, minStopDistance, maxStopDistance);
    ...
    const formattedStop = {
      ...stop,
      ...find(stationList[agencyKey], ['stop_id', stop.stop_id])
    };
    
    return {
      stop_id: formattedStop.stop_id,
      stop_code: noStopCode ? null : formattedStop.stop_code ?? formattedStop.stop_id,
      name: formattedStop.stop_name,
      distance: stoptime.shape_dist_traveled,
      direction_id: trip.direction_id,
      timezone: formattedStop.stop_timezone ?? defaultTimezone,
      stop_short_name: formattedStop.stop_short_name ?? formattedStop.stop_name,
      state: formattedStop.state ?? "",
      major: formattedStop.major,
    };
  });
};
 ...
 */
const getDataforChart = (config, page) => {
  ...
  const agency = db.prepare('SELECT DISTINCT agency_timezone FROM agency').all();
  const agencyTimezone = agency[0].agency_timezone;

  var calendars = [];
  for (let day = 0; day < days.length; day++) {
    const calendar = db.prepare(`SELECT DISTINCT service_id FROM calendar WHERE start_date <= ? AND end_date >= ? AND ${sqlString.escapeId(daysOfWeek[day])} = 1`).all(chartDates[day], chartDates[day]);
  ...
  for (let day = 0; day < days.length; day++) {
    ...
    for (const i in routeIds) {
      const trips = db.prepare(`SELECT service_id, trip_id, trip_short_name, direction_id, shape_id FROM trips where route_id = ? AND service_id IN (${serviceIds[day].map(() => '?').join(', ')})`, )
        .all(...[ routeIds[i], ...serviceIds[day] ]);
  ...
  let stations = getStationsFromTrip(longestTrip, agencyTimezone, config);
  const tripDistance = max(map(stations, 'distance'));
  ...
  if (size(directionGroups) > 1) {
    ...
    const stationsOppositeDirection = getStationsFromTrip(longestTripOppositeDirection, agencyTimezone, config);
  ...
  const tzCounts = stations
    .map(station => station.timezone)
    .reduce((result, timezone) => {
      result[timezone] = (result[timezone] ?? 0) + 1;
      return result;
    }, {});
  const tzMax = Math.max.apply(null, Object.values(tzCounts));
  const chartTimezone = Object.keys(tzCounts).find(key => tzCounts[key] === tzMax);
 ...
 */
export async function generateChartHTML(config, urlSlug) {
  const agencyKey = config.agencies[0].agency_key;
  const route = find(pageList[agencyKey], ['url_slug', urlSlug]);

  const chartData = getDataforChart(config, route);
