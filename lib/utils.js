...
function calculateDistanceMi(lat1, lon1, lat2, lon2) {
  ...
  dist = dist * 60 * 1.1515;
  return Math.min(30, Math.max(15, dist));
}
...
const getDataforChart = async (config, routeId) => {
  const routeIds = [routeId].concat(config.combine_routes && config.combine_routes[routeId] ? config.combine_routes[routeId] : []);
  ...
  for (let day = 0; day < days.length; day++) {
    ...
    if (calendar.length === 0) {
      throw new Error(`No calendars found for route ${routeIds[0]} on ${moment(chartDates[day], 'YYYYMMDD').format('MMM D, YYYY')}`);
    }
  }
  ...
  var tripsByDay = [];
  for (let day = 0; day < days.length; day++) {
    const trips = await db.all(`SELECT service_id, trip_id, trip_headsign, trip_short_name, direction_id, shape_id FROM trips where route_id IN (${routeIds.map(() => '?').join(', ')}) AND service_id IN (${serviceIds[day].map(() => '?').join(', ')})`, [
      ...routeIds,
      ...serviceIds[day]
    ]);
    tripsByDay.push(trips);
  }

  if (tripsByDay.flat().length === 0) {
    throw new Error(`No trips found for route ${routeIds[0]} on ${moment(chartDates[0], 'YYYYMMDD').format('MMM D, YYYY')}`);
  }
  ...
  const longestTrip = config.overrideStopList && config.overrideStopList[routeId] ? config.overrideStopList[routeId] : findLongestTrip(trips);
  ...
  if (size(directionGroups) > 1) {
    ...
    const longestTripOppositeDirection = config.overrideStopList && config.overrideStopList[routeId] ? {
      ...config.overrideStopList[routeId],
      direction_id: oppositeDirection,
      stoptimes: longestTrip.stoptimes.reverse(),
    } : findLongestTrip(directionGroups[oppositeDirection]);
