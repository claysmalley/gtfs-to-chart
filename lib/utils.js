...
const getDataforChart = async (config, routeId) => {
  ...
  const notes = [];
  const chartDates = [0, 1, 2].map((day) => moment(config.chartDate, 'YYYYMMDD').add(day, 'days').format('YYYYMMDD'));
  const daysOfWeek = chartDates.map((date) => moment(date, 'YYYYMMDD').format('dddd').toLowerCase());
  const calendars = await db.all(`SELECT DISTINCT service_id FROM calendar WHERE start_date <= ? AND end_date >= ? AND ${sqlString.escapeId(daysOfWeek[0])} = 1`, [
    chartDates[0],
    chartDates[0]
  ]);

  if (calendars.length === 0) {
    throw new Error(`No calendars found for route ${routeId} on ${moment(chartDates[0], 'YYYYMMDD').format('MMM D, YYYY')}`);
  }

  const serviceIds = calendars.map(calendar => calendar.service_id);
  const trips = await db.all(`SELECT service_id, trip_id, trip_headsign, trip_short_name, direction_id, shape_id FROM trips where route_id = ? AND service_id IN (${serviceIds.map(() => '?').join(', ')})`, [
    routeId,
    ...serviceIds
  ]);
  ...
  await Promise.all(trips.map(async trip => {
    ...
    trip.stoptimes = stoptimes.filter(stoptime => isTimepoint(stoptime)).map(
      (stoptime) => {
        stoptime['arrival_time_utc'] = parseTime(stoptime.arrival_time, chartDates[0], 'America/New_York');
        stoptime['departure_time_utc'] = parseTime(stoptime.departure_time, chartDates[0], 'America/New_York');
        return stoptime;
      }
    );
  }));
