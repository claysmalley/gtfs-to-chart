...
const gtfsToChart = async initialConfig => {
  ...
  return Promise.all(config.agencies.map(async agency => {
    ...
    const agencyPages = pageList[agencyKey];
    ...
    const bar = progressBar(`${agencyKey}: Generating charts [:bar] :current/:total`, { total: agencyPages.length }, config);
    ...
    for (const route of agencyPages) {
      outputStats.charts += 1;

      try {
        const html = await generateChartHTML(config, route.url_slug, agencyTimezone);
        const htmlPath = path.join(exportPath, sanitize(`${route.url_slug}.html`));
    ...
    config.assetPath = '';
    const html = await generateOverviewHTML(config, agencyPages);
