...
const gtfsToChart = async initialConfig => {
  ...
  return Promise.all(config.agencies.map(async agency => {
    ...
    const bar = progressBar(`${agencyKey}: Generating charts [:bar] :current/:total`, { total: routeList.length }, config);
    ...
    config.assetPath = '';
    const html = await generateOverviewHTML(config, routeList);
